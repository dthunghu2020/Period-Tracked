...
public abstract class WeekView extends BaseWeekView {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mItems.size(); i++) {
            ...
            if (hasScheme) {
                ...
                //Có tiếp tục vẽ bản vẽ đã chọn trên DrawScheme không
                if (isSelected) {
                    isDrawSelected = onDrawSelected(canvas, calendar, x, true);
                }
                if (isDrawSelected || !isSelected) {
                    //将画笔设置为标记颜色
                    //Đặt cọ làm màu đánh dấu
    ...
    public boolean onLongClick(View v) {
        ...
        if (mDelegate.isPreventLongPressedSelected()) {//如果启用拦截长按事件不选择日期
            //Nếu tính năng chặn các sự kiện báo chí dài được bật, không có ngày nào được chọn
            if (mDelegate.mCalendarLongClickListener != null) {
